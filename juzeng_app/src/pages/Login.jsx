...
const Login = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/token/', payload, {
        headers: {
    ...
    <div className="min-h-screen bg-cover bg-center flex items-center justify-center">
      <div className="flex flex-col md:flex-row max-w-4xl w-full bg-white rounded-lg shadow-lg overflow-hidden m-4">
        ...
        <div className="w-full md:w-1/2 bg-gray-100 flex justify-center">
          <img src="/images/login_background.png" alt="Login Background" className="object-cover w-full h-full md:rounded-l-lg" />
        </div>

        {/* Form Section */}
        <div className="w-full md:w-1/2 p-6 flex flex-col justify-center">
          <h2 className="text-2xl font-semibold text-center mb-4">Sign in to your account</h2>
          <p className="text-center mb-6">Welcome back! Login with your data provided by JuzEng :)</p>
          ...
          <form onSubmit={handleLogin}>
            <div className="mb-3">
              <label className="mb-1 block text-gray-700"><strong>Email</strong></label>
              <input
                type="email"
            ...
            <div className="mb-3">
              <label className="mb-1 block text-gray-700"><strong>Password</strong></label>
              <input
                type="password"
                className="form-control w-full px-4 py-2 border rounded focus:outline-none"
            ...
            <div className="text-center">
              <button type="submit" className="btn btn-primary btn-block w-full bg-blue-500 text-white py-2 rounded-lg hover:bg-blue-600">Sign Me In</button>
            </div>
          </form>
        </div>
      </div>
